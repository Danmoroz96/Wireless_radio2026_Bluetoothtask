...
Technical AnalysisRelationship Between RSSI and DistanceThe Received Signal Strength Indicator (RSSI) measured in dBm (decibels per milliwatt) is a logarithmic representation of signal power.
Signal Decay: Radio waves decrease in energy exponentially with distance. A strong signal of -65 dBm correlates to a close proximity of roughly 2 meters.Distance Formula: Estimated distance ($d$) can be calculated using the Log-Distance Path Loss Model:
